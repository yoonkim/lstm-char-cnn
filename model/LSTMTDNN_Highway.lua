...
function LSTMTDNN.lstmtdnn(rnn_size, n, dropout, word_vocab_size, word_vec_size, char_vocab_size, char_vec_size,
	...
	if L == 1 then
		...
		local cnn_output1 = char_cnn(char_vec) --output is batch_size x cnn_output_size
		local cnn_output2 = nn.Tanh()(nn.Linear(cnn_output_size, cnn_output_size)(cnn_output1))
		local cnn_transform_gate = nn.Sigmoid()(nn.AddConstant(-2)(nn.Linear(cnn_output_size, cnn_output_size)(cnn_output1)))
		local cnn_carry_gate = nn.AddConstant(1)(nn.MulConstant(-1)(cnn_transform_gate))
		local cnn_transform = nn.CMulTable()({cnn_transform_gate, cnn_output2})
		local cnn_carry = nn.CMulTable()({cnn_carry_gate, cnn_output1})
		local cnn_output = nn.CAddTable()({cnn_carry, cnn_transform})
		input_size_L = cnn_output_size
	        if use_words == 1 then
